...
return route.path.includes('/utilisateur')
})

const userName = computed(() => {
return store.currentUser || 'Invité'
})

const userInitial = computed(() => {
if (!store.currentUser) return '?'
return store.currentUser.charAt(0).toUpperCase()
})

const profilePath = computed(() => {
if (store.currentUser) return /utilisateur/${store.currentUser}
return '/login'
})
</script>