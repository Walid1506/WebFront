    ...
    <header class="bg-slate-900 text-white border-b border-slate-800 sticky top-0 z-50 h-16 shadow-md">
      <div class="container mx-auto px-4 h-full flex items-center justify-between">
        ...
        <div class="flex-1 flex justify-start items-center gap-2">
          <NuxtLink v-if="$route.path.includes('/utilisateur')" to="/" class="px-3 py-1 bg-slate-800 border border-slate-700 rounded-lg text-xs font-bold text-gray-200 hover:bg-slate-700 transition flex items-center gap-1">
            <span>←</span> Retour
          </NuxtLink>
        ...
        <div class="flex-1 flex justify-end items-center gap-3">
          ...
          <NuxtLink 
            :to="store.currentUser ? `/utilisateur/${store.currentUser}` : '/login'" 
            class="block rounded-full ring-2 ring-slate-700 overflow-hidden w-8 h-8 bg-slate-800 text-green-400 flex items-center justify-center font-bold hover:ring-green-500 transition cursor-pointer"
          >
