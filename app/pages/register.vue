...
const schema=yup.object({pseudo:yup.string().required("Le pseudo est requis").min(3,"3 caractères minimum"),password:yup.string().required("Le mot de passe est requis").min(4,"4 caractères minimum"),confirmPassword:yup.string().required("La confirmation est requise").oneOf([yup.ref("password")],"Les mots de passe ne correspondent pas")})
...
const onSubmit=handleSubmit(values=>{const ok=store.register(values.pseudo,values.password);if(ok)router.push("/");else setErrors({pseudo:"Ce pseudo est déjà utilisé."})})
</script>
