  ...
  <div class="w-full max-w-sm bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100 relative">
    ...
    <div class="px-8 pt-10 pb-6 text-center relative z-10">
      ...
      <h2 class="text-3xl font-black text-slate-900 tracking-tight">
        Fit<span class="text-transparent bg-clip-text bg-gradient-to-r from-green-500 to-emerald-700">Track</span>
      </h2>
      <p class="text-sm font-medium text-gray-400 mt-2">Prêt à changer de vie et à nous rejoindre ?</p>
    </div>

    <form @submit="handleLogin" class="px-8 pb-10 space-y-5">
      <div>
        <label for="pseudo" class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 pl-1">Ton Prénom</label>
        <div class="relative group">
          <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            <UIcon name="i-heroicons-user" class="text-gray-400 group-hover:text-green-500 transition-colors text-xl" />
          </div>
      ...
      <div>
        <label for="password" class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 pl-1">Mot de passe</label>
        <div class="relative group">
          <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            <UIcon name="i-heroicons-lock-closed" class="text-gray-400 group-hover:text-green-500 transition-colors text-xl" />
          </div>
          <input id="password" v-model="password" v-bind="passwordAttrs" type="password" placeholder="••••••••" class="block w-full pl-11 pr-4 py-4 border-2 rounded-2xl text-gray-900 placeholder-gray-400 bg-gray-50/50 focus:bg-white focus:outline-none focus:border-green-500 transition duration-300 font-medium" :class="errors.password ? 'border-red-500 bg-red-50' : 'border-gray-100'" />
        </div>
        <p v-if="errors.password" class="text-red-500 text-xs font-bold mt-1 pl-1">{{ errors.password }}</p>
      </div>

      <button type="submit" class="w-full bg-gradient-to-r from-slate-900 to-slate-800 text-white font-bold py-4 px-4 rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed" :disabled="Object.keys(errors).length > 0">
        Se connecter
      </button>

      <div class="text-center mt-6 pt-4 border-t border-gray-100">
        <p class="text-xs text-gray-400 font-medium mb-2">Tu n'as pas encore de compte ?</p>
        <NuxtLink to="/register" class="text-sm font-black text-green-600 hover:text-green-700 hover:underline transition">Créer un compte</NuxtLink>
      </div>
    </form>
  </div>
</template>
