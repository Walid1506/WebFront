...
const { defineField, handleSubmit, errors } = useForm({
  ...
  initialValues: {
    pseudo: store.currentUser || "",
  },
});

const [pseudo, pseudoAttrs] = defineField("pseudo");

const sauvegarder = handleSubmit((values) => {
      ...
      <form @submit="sauvegarder" class="p-8 flex flex-col items-center gap-6">
        ...
        <div class="w-full">
          ...
          <input
            ...
            v-bind="pseudoAttrs"
            type="text"
            class="w-full border rounded-lg p-3 font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-green-400 focus:border-transparent transition"
            :class="errors.pseudo ? 'border-red-500 bg-red-50' : 'border-gray-300'"
            placeholder="Entre ton nouveau pseudo"
          />
          <p v-if="errors.pseudo" class="text-red-500 text-xs font-bold mt-1 ml-1">
            {{ errors.pseudo }}
          </p>
        </div>
        ...
        <div class="flex gap-3 w-full mt-2">
          ...
          <button
            type="submit"
            class="flex-1 py-3 rounded-xl font-bold text-white bg-green-500 hover:bg-green-600 shadow-md hover:shadow-lg transition transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
